...
export default function ReportPage() {
  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        const parsed: ReportJSON = JSON.parse(cached);
        if (parsed.metadata.problem === savedProblem) {
          setReport(parsed);
          setStatus("done");
  ...
  useEffect(() => {
    if (!problem || hasFetchedRef.current) return;
    hasFetchedRef.current = true;
    runPipeline(problem);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [problem]);

  const runPipeline = async (prob: string) => {
    setStatus("stage1");
    setAgentLog([]);
    setError(null);
    setReport(null);
          ...
          if (eventType === "agent_log") {
            ...
            setAgentLog((prev) => {
              const idx = prev.findIndex(
                (e) => e.agent === entry.agent && e.status === "running"
              );
              if (idx !== -1 && entry.status === "complete") {
                const next = [...prev];
                next[idx] = entry;
                return next;
              }
  ...
  const handleSectionSave = (sectionId: SectionId, updated: SectionData, instruction: string) => {
    ...
    const currentContent = report[sectionId];
    ...
    setReport((prev) =>
      prev ? { ...prev, [sectionId]: updated } : prev
    );
  };
  ...
  const getOtherSections = (editingId: SectionId): Record<string, SectionData> => {
    ...
    const all: Record<string, SectionData> = {
      problemBreakdown: report.problemBreakdown,
      stakeholders: report.stakeholders,
      solutionApproach: report.solutionApproach,
    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-orange-50 font-sans pb-20 text-slate-900">
      ...
      {editingSection && report && (
        <SectionEditor
          ...
          currentContent={report[editingSection]}
          problem={problem}
